# Balash Guide State (skill version: v3.1, budgeted discovery)
...
## Provisional product defaults (Guide-chosen, cheap, reversible — not asked)

- Status may change directly between any pair, including reopening `done`. Revisit if a workflow
  invariant is ever stated.
- Empty title is rejected; empty description is allowed. Revisit if the user states otherwise.
- Reassignment after initial assignment is allowed. Revisit if the user states otherwise.

## Invariants

- Task always has stable ID, title (non-empty, per provisional default), description, assignee
  (nullable), status.
...
- Persistence seam: `TaskRepository.{next_identity,save,get,list_all}`; one JSON implementation;
  AST-checked (by the Worker's own test) that domain/service import no storage format.
- IDs `t1, t2, ...` via a persisted `next_id` counter (survives restart, no reuse).
- Assignee captured per-assignment, no roster — same acceptance as the v3 run.

## Open Guide TODO

- [ ] Watch whether AI-agent identity (stage 2) reuses or duplicates the `Assignee` pattern.
...
**Why now:** Entire stage-2 request. Discovery resolved the 3 highest-impact open decisions
(agent identity, result schema, history-vs-overwrite); everything else is a provisional default
or a technical freedom — see log.
...
auto-triggering execution on assignment (provisional default: stays separate), agent
update/rename (provisional default: create-once this stage).
...
**Why now:** Entire stage-3 request; discovery's one question (no automatic migration) was the
only genuinely open decision; the rest is already specified by the request text.
...
**Why now:** Entire stage-4 request; discovery's 3-question budget resolved cycle handling,
creation-only-vs-editable prerequisites, and reopen behavior. Blocked-message detail is a
provisional default (title+status included); "does the block also cover direct-to-done" is
already grounded in the request text (it names only in-progress and AI execution).
...
- [ ] Moving to `in progress` while a prerequisite is not `done` is rejected, naming every
      blocking prerequisite's id, title, and status (provisional default); task stays in its
      current status.
...
**stage 3: met.** Independently re-verified: 145/145 tests pass on my own re-run; `git diff
stage-2` on domain/service/repository/providers empty, matching the Worker's claim.
